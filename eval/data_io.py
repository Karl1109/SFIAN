...
def get_side1_image_pairs(data_dir, suffix_gt='real_B', suffix_side1='fake_B'):
    gt_list = glob.glob(os.path.join(data_dir, '*{}.png'.format(suffix_gt)))
    side1_pred_list = [ll.replace(suffix_gt, suffix_side1) for ll in gt_list]
    assert len(gt_list) == len(side1_pred_list)
    side1_imgs, gt_imgs = [], []
    for side1_path, gt_path in zip(side1_pred_list, gt_list):
        side1_imgs.append(imread(pred_path))
        gt_imgs.append(imread(gt_path, thresh=127))
    return side1_imgs, gt_imgs
